...
import {get_shangquanqb} from "../../actions"
import {connect} from "react-redux";
@connect(
    (state)=>({
        shangquanqb:state.shangquanqb
    })
)
...
export default class Zzz extends Component{
    ...
    componentWillMount(){
        ...
        dispatch(get_shangquanqb("/getshangquanqb",dispatch))
    }
    ...
    render(){
        ...
        const {shangquanqb} =this.props;
        var html=null;
        const {location} = this.props;
        
        var routes = location.query.title;
       


        if(shangquanqb.length>0){
            html=(
                shangquanqb.map((item,idx)=>{
                    return(
