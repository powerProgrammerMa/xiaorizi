...
import {get_bangdancdtop10} from "../../actions";
@connect(
    (state)=>({
        bangdancdtop10:state.bangdancdtop10
        
    })
)

export default class Bangdancd extends Component{

    componentWillMount(){
        const {dispatch} =this.props;
        dispatch(get_bangdancdtop10("/getbangdancdtop10",dispatch))
    ...
    render(){
        ...
        const {bangdancdtop10}=this.props;
      
        var html=null;

        if(bangdancdtop10.length>0){
            html=(


                bangdancdtop10.map((item,idx)=>{
